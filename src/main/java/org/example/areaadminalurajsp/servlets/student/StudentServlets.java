...
public class StudentServlets extends HttpServlet {
    ...
    private void insertStudentUnblockedRequest(HttpServletRequest request, String token) throws IOException {
        String pageUnblock = request.getParameter("pageUnblock");
        List<StudentReadDTO> allStudent = studentService.getAllStudent(Integer.valueOf(pageUnblock), token);
    ...
    private void insertStudentBlockedRequest(HttpServletRequest request, String token) throws IOException {
        String pageBlock = request.getParameter("pageUnblock");
        List<StudentBlockedReadDTO> allStudent = studentService.getAllStudentBlocked(Integer.valueOf(pageBlock), token);
